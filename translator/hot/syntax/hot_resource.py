...
class HotResource(object):
    ...
    def handle_life_cycle(self):
        ...
        # create HotResrouce for each interface used for deployment:  create, start, configure
        ...
        node_name = self.name           # hold the original name since it will be changed during the transformation
        ...
        # create the set of SoftwareDeployment and SoftwareConfig for the interface operations
        for interface in self.nodetemplate.tpl_interfaces:
            if interface.name in interfaces_deploy_sequence:
                ...
                hot_resources.append(HotResource(self.nodetemplate, 
                                                 ...
                                                 {'config':{'get_file':interface.implementation}}))
        ...
        # Add dependencies for the set of HOT resources in the sequence defined in interfaces_deploy_sequence
        # todo: find some better way to encode this implicit sequence
        group = {}
        for op, hot in deploy_lookup.iteritems():
            op_index = interfaces_deploy_sequence.index(op)    # position to determine potential preceding nodes
            for preceding_op in reversed(interfaces_deploy_sequence[:op_index]):
    ...
    def handle_hosting(self):
        ...
        # from the TOSCA nodetemplate, traverse the relationship chain down to the server
        if self.type=='OS::Heat::SoftwareDeployment':
            # skip if already have hosting 
            host_server = self.properties.get('server')
            if host_server==None:
                host_server = self.bottomofchain().properties['server']['get_resource']
    ...
    def topofchain(self):
        ...
        if dependent==None:
            ...
            return dependent.topofchain()
            
    # todo:  better to traverse using the relationship requirement:host in TOSCA
    def bottomofchain(self):
        if len(self.depends_on)==0:
            return self
        else:
            for preceding in self.depends_on:
                return preceding.bottomofchain()
